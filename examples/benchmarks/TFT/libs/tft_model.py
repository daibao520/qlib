...
# Layer definitions.
concat = tf.keras.ops.concatenate
stack = tf.keras.ops.stack
K = tf.keras.backend
...
expand_dims = tf.keras.ops.expand_dims
...
def get_decoder_mask(self_attn_inputs):
    ...
    """
    # len_s = tf.keras.ops.shape(self_attn_inputs)[1]
    # bs = tf.keras.ops.shape(self_attn_inputs)[:1]
    # mask = K.cumsum(tf.eye(len_s, batch_shape=bs), 1)
    # return mask

    def mask_fn(x):
        batch_size = tf.keras.ops.shape(x)[0]
        seq_len = tf.keras.ops.shape(x)[1]
        single_mask = tf.linalg.band_part(tf.ones((seq_len, seq_len)), -1, 0)
        mask = tf.tile(tf.expand_dims(single_mask, 0), [batch_size, 1, 1])
        return mask

    return Lambda(mask_fn)(self_attn_inputs)
...
class ScaledDotProductAttention():
    ...
    def __call__(self, q, k, v, mask):
        ...
        """
        tt = tf.keras.ops.shape(k)[-1]
        # temper = tf.sqrt(tf.cast(tf.shape(input=k)[-1], dtype='float32'))
        temper = tf.sqrt(tf.cast(tt, dtype='float32'))
...
class InterpretableMultiHeadAttention():
    ...
    def __call__(self, q, k, v, mask=None):
        ...
        for i in range(n_head):
            ...
            attns.append(attn)
        head = stack(heads) if n_head > 1 else heads[0]
        attn = stack(attns)
...
class TemporalFusionTransformer(object):
    ...
    def get_tft_embeddings(self, all_inputs):
        ...
        regular_inputs, categorical_inputs = all_inputs[:, :,
    ...
    def _build_base_graph(self):
        ...
        def static_combine_and_mask(embedding):
            ...
            _, num_static, _ = embedding.shape
            ...
            sparse_weights = expand_dims(sparse_weights, axis=-1)
        ...
        def lstm_combine_and_mask(embedding):
            ...
            # Add temporal features
            _, time_steps, embedding_dim, num_inputs = embedding.shape
            ...
            sparse_weights = tf.keras.layers.Activation('softmax')(mlp_outputs)
            sparse_weights = expand_dims(sparse_weights, axis=2)
